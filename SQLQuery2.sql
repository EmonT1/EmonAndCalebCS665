...
CREATE TRIGGER [dbo].[Trigger_TimeCards] ON TimeCards AFTER UPDATE AS BEGIN IF NOT EXISTS (SELECT 1 FROM Employee WHERE EID IN (SELECT EID FROM inserted)) BEGIN RAISERROR('Employee ID not found in Employee table', 16, 1); ROLLBACK TRANSACTION; END END;
...
(
	...
	[TaskID] INT NOT NULL, 
	CONSTRAINT [FK_TimeEntries_TimeCards] FOREIGN KEY ([TimeCardID]) REFERENCES TimeCards,
	CONSTRAINT [FK_TimeEntries_Tasks] FOREIGN KEY ([TaskID]) REFERENCES Tasks 
);
